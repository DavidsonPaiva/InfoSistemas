...
begin
  ...
  try
    ...
    if not FParametro.emailCopia.Trim.isempty then
      IdMessage.CCList.EMailAddresses := FParametro.emailCopia.Trim;
    ...
    if Assigned(FParametro.anexo) then
    begin
      for I := 0 to Pred(FParametro.anexo.Count) do
      begin
        if FileExists(FParametro.anexo.Strings[I]) then
        begin
          try
            DeleteFile(pchar(FParametro.anexo.Strings[I]));
          except
          end;
        end;
      end;
